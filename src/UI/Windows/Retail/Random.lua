	...
	OnInit = function(self, handlers)
		...
		local function SearchRecursivelyForValue(group, results, func, field, value)
			if group.visible and not (group.saved or group.collected) then
				if group.g then
					for i, subgroup in ipairs(group.g) do
						SearchRecursivelyForValue(subgroup, results, func, field, value)
					end
				end
		...
		self.RebuildRandom = function(self, no)
			...
			for i=1,#self.data.options do
				tinsert(self.data.g, self.data.options[i]);
			end
			...
			if not primePending then
				...
				if totalWeight > 0 and #temp > 0 then
					...
					if selected then
						app.PrintDebug("rows",#self.data.g)
				...
				rerollOption.text = "Click to open /att"
				local rerollClick = rerollOption.OnClick
				rerollOption.OnClick = function(row, button)
					app.ToggleMainList()
					local method = self.RandomSearchFilter or appName;
					rerollOption.text = L.REROLL_2 .. (method ~= appName and L[method:upper()] or method);
					rerollOption.OnClick = rerollClick
					return true
				end
			end
			self:Rebuild()
		...
		self.Reroll = function(self)
			app.CallbackHandlers.Callback(self.RebuildRandom, self)
		end
